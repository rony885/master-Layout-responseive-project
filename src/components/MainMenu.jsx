      ...
      <div className="container mx-auto flex justify-between items-center">
        ...
        <div className="w-[80%] hidden md:flex justify-end items-center">
          <ul className="flex gap-3">
            {menuItems.map(item => (
              <li key={item.id}>
                <Link to={item.link} className="text-lg font-medium text-white font-roboto hover:text-black">
                  {item.title}
                </Link>
              </li>
            ))}
          ...
          <ul className="flex flex-col gap-3 items-center">
            {menuItems.map(item => (
              <li key={item.id}>
                <Link
                  to={item.link}
                  className="text-lg font-medium text-white font-roboto hover:text-black"
